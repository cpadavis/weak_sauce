...
def fitModel(*args):
    ...
    if fitType == "DES":
        foldername = (
            "/nfs/slac/g/ki/ki19/des/mbaumer/ccd_mg_model_fits/des_chip04_maxit"
            + str(maxit_arg)
            + "_step"
            + str(step_arg)
            + "_decay"
        ...
        foldername = (
            "/nfs/slac/g/ki/ki19/lsst/mbaumer/ccd_mg_model_fits/lsst_amp03_maxit"
            + str(maxit_arg)
            + "_step"
            + str(step_arg)
            + "_decay"
        ...
        foldername = (
            "/nfs/slac/g/ki/ki19/lsst/mbaumer/ccd_mg_model_fits/validation_gaussian_synthetic4_lsst_amp03_maxit"
            + str(maxit_arg)
            + "_step"
            + str(step_arg)
            + "_decay"
    ...
    if os.path.exists(foldername):
        raise IOError(
            "folder for this config already exists! delete if you want to do again!"
        )

    if fitType == "DES":
        full_amp_img = fits.getdata(
            "/nfs/slac/g/ki/ki19/des/mbaumer/DES_flatcor_supercal/coadds/coadd_r_04.fits"
        )
        ...
        full_amp_img = np.load(
            "/u/ki/mbaumer/random_pixel_size/weak_sauce/data/lsst_ultraflat_75ke_amp3.npy"
        )
    ...
    mask = np.logical_not(
        np.logical_and(
            small_img < np.mean(small_img) + sig * sigma,
            small_img > np.mean(small_img) - sig * sigma,
        )
    )
    locs = np.where(mask)
    ...
    # unperturbed
    data_like_source = Source(
        num_x=small_img.shape[1] + 1, num_y=small_img.shape[0] + 1
    )
